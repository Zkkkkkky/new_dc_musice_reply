function run_mitsume_matrix(commands, log_name, require_latch_clear)
    local output = assert(io.open(log_name, "w"))
    local frame = 0
    local track = 0
    local phase_frame = 0
    local init_hits = 0
    ...
    memory.registerexec(0xB500, 1, function()
        if not injected or memory.readbyte(0x046A) == 0xD3 then return end
        if allowed_command ~= nil then
            memory.writebyte(0x004C, allowed_command)
            allowed_command = nil
        elseif memory.readbyte(0x004C) >= 0x80 then
            memory.writebyte(0x004C, 0xFF)
        end
    end)
    ...
    local function check_track()
        ...
        local ok = clean_state and initialized and played and
            return_hits - before_return >= 10 and
            (not require_latch_clear or clear_hits - before_clear >= 10) and
            apu_writes > before_apu and reg("pc") >= 0x8000
    ...
    output:write(string.format(
        "TIMEOUT frame=%d track=%d failures=%d\n", frame, track, failures))
    output:close()
    emu.exit()
end
